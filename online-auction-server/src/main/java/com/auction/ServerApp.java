 ...
 * <p>
 * Lớp này đóng vai trò là trái tim của hệ thống, chịu trách nhiệm:
 * <ul>
...
public class ServerApp {
     ...
     * @param args Các tham số dòng lệnh truyền vào khi chạy chương trình (hiện tại không dùng đến).
     */
    public static void main(String[] args) {
        ...
        // 2. Chuẩn bị sẵn một Thread Pool tối đa 20 luồng
        ExecutorService pool = Executors.newFixedThreadPool(MAX_THREADS);

        // 3. Mở cổng mạng an toàn với try-with-resources (tự động đóng khi Server tắt)
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            logger.info("Server started on port {}", PORT);
            logger.info("Waiting for client connections...");

            // 4. Vòng lặp vô tận: Cứ có khách đến là quăng cho Thread Pool xử lý
            while (true) {
