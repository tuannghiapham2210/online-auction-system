...
/**
 * Lớp dịch vụ quản lý các nghiệp vụ logic liên quan đến phiên đấu giá (Auction).
 */
public class AuctionService {
  private static final Logger logger = LoggerFactory.getLogger(AuctionService.class);

  /**
   * Lớp chứa kết quả xử lý nghiệp vụ đấu giá để trả về cho ClientHandlers.
   */
  public static class AuctionResult {
    ...
    /**
     * Khởi tạo kết quả AuctionResult.
     */
  ...
  /**
   * Xử lý nghiệp vụ thêm một sản phẩm đấu giá mới vào hệ thống.
   *
   * @param request Đối tượng JSON chứa thông tin sản phẩm từ client.
   * @return Đối tượng {@link AuctionResult} chứa phản hồi.
   */
  public AuctionResult processAddItem(JsonObject request) {
    try {
      ...
      // FIX: Truyền thẳng null thay vì tạo biến dư thừa gây cảnh báo IntelliJ
      Item newItem = ItemFactory.createItem(type, name, startingPrice, null, sellerId, "");
      ...
      if (isSuccess) {
        ...
        int insertedId = -1;
        if (!allItems.isEmpty()) {
          // FIX: Thay thế cách tính size cũ bằng hàm getLast() theo khuyến nghị của IDE
          insertedId = allItems.getLast().getId();
        }
  ...
  /**
   * Phát sinh thông báo broadcast khi một sản phẩm được kích hoạt hiển thị công khai.
   */
  ...
  /**
   * Lấy toàn bộ danh sách sản phẩm hiện có từ tầng dữ liệu.
   */
  ...
  /**
   * Khởi chạy kích hoạt một phiên đấu giá chuyển sang trạng thái hoạt động trực tiếp.
   */
  public AuctionResult processOpenAuction(int itemId, int userId, String role) {
    try {
      ...
      if ("ADMIN".equalsIgnoreCase(role) || item.getSellerId() == userId) {
        ...
        logger.warn("Unauthorized OPEN_AUCTION_REQUEST for itemId={} by userId={}, role={}",
            itemId, userId, role);
        return new AuctionResult(errorMsg, null);
      }
    } catch (Exception e) {
  ...
  /**
   * Xử lý gỡ bỏ hoặc hủy bỏ hoàn toàn một phiên đấu giá khỏi hệ thống mạng.
   */
    ...
    try {
      ...
      if ("ACTIVE".equalsIgnoreCase(item.getStatus())
        ...
        try {
          ...
          java.time.LocalDateTime endTime =
              java.time.LocalDateTime.parse(item.getEndTime(), formatter);
          if (java.time.LocalDateTime.now().isBefore(endTime)) {
            isLive = true;
          } else {
            itemDao.updateAuctionStatus(itemId, "FINISHED");
            logger.info("[SERVER] Đã tự động chốt trạng thái FINISHED cho itemId={} do hết hạn.",
                itemId);
          }
        } catch (Exception e) {
      ...
      if (isLive) {
        ...
        errorMsg.addProperty("message",
            "Không được phép gỡ bỏ sản phẩm khi phiên đấu giá đang diễn ra trực tiếp!");
        logger.warn("[SERVER] Chặn hành vi gỡ sản phẩm đang đấu giá: itemId={}, bởi userId={}",
            itemId, userId);
        return new AuctionResult(errorMsg, null);
      }
      ...
      if ("ADMIN".equalsIgnoreCase(role) || item.getSellerId() == userId) {
        ...
        if (success) {
          ...
          broadcastMsg.addProperty("itemId", itemId);
          broadcastMsg.addProperty("message", "Sản phẩm '" + item.getName()
              + "' đã bị gỡ bỏ khỏi hệ thống.");
          logger.info("Product ID {} successfully removed by userId={}, role={}",
              itemId, userId, role);
  ...
  /**
   * Đóng phiên khẩn cấp lập tức, thực hiện chốt sổ tính toán người dẫn đầu chiến thắng.
   */
  public AuctionResult processStopAuction(int itemId, int userId, String role) {
    try {
      logger.info("[SERVER] Nhận yêu cầu dừng phiên khẩn cấp từ client: "
              + "itemId={}, userId={}, role={}",
          itemId, userId, role);
      ItemDao itemDao = new ItemDao();
      Item item = itemDao.getItemById(itemId);
      ...
      if (statusUpdated) {
        ...
        logger.info("[SERVER] Cập nhật CSDL: Chuyển item {} sang FINISHED và chốt giờ về {}",
            itemId, nowStr);
        ...
        logger.info("[SERVER] Broadcast sự kiện kết thúc thầu sớm: Item={}, Winner={}, Price=${}",
  ...
  /**
   * Truy xuất toàn bộ danh sách lịch sử các lượt thầu của một sản phẩm.
   */
