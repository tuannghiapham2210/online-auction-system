 ...
 */
public class PasswordChangeNetworkRequest {
  private static final Logger logger = LoggerFactory.getLogger(
      PasswordChangeNetworkRequest.class);
   ...
   *                    Application Thread
   */
  public static void sendChangePasswordRequestAsync(
      int userId,
      String oldPassword,
      String newPassword,
    ...
    new Thread(() -> {
      ...
      try (Socket socket = new Socket(HOST, PORT);
        ...
        String message = response.has("message")
            ? response.get("message").getAsString() : "Lỗi đổi mật khẩu.";
