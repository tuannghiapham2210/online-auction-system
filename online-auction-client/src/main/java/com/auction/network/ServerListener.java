...
public class ServerListener implements Runnable {
  ...
  private final BidRoomController controller;
  ...
  private void registerEventHandlers() {
    ...
    eventHandlers.put("AUCTION_FINISHED", json -> {
      int itemId = json.get("itemId").getAsInt();
      String winnerUsername = json.has("winnerUsername")
          ? json.get("winnerUsername").getAsString() : "Không có";
    ...
    eventHandlers.put("ERROR", json -> {
      String errorMessage = json.has("message")
          ? json.get("message").getAsString() : "Đã có lỗi xảy ra!";
