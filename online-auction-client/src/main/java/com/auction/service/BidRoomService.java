 ...
 *
 * <p>Lớp này chịu trách nhiệm kiểm tra tính hợp lệ của các yêu cầu đặt giá (Validation)
...
public class BidRoomService {
   ...
   */
  public void validateBid(double bidAmount, double currentPrice, double stepPrice,
                          double userBalance) {
    double minBid = currentPrice + stepPrice;
    if (bidAmount < minBid) {
      throw new IllegalArgumentException("Giá đặt tối thiểu phải là $"
          + NumberUtil.format(minBid));
    }
   ...
   * @param autoBidIncrementAmount Số tiền sẽ tự động cộng thêm khi bị vượt mặt.
   * @param currentPrice           Giá hiện tại của sản phẩm.
   * @param stepPrice              Bước giá quy định của sản phẩm.
   * @param userBalance            Số dư khả dụng của người dùng.
  ...
  public void validateAutoBid(
      double maximumAutoBidBudget,
      double autoBidIncrementAmount,
      double currentPrice,
      double stepPrice,
      double userBalance) {
    double minMaxBid = currentPrice + stepPrice;
    if (maximumAutoBidBudget < minMaxBid) {
      throw new IllegalArgumentException("Giá max phải lớn hơn hoặc bằng giá tối thiểu tiếp theo ($"
          + NumberUtil.format(minMaxBid) + ")!");
    }

    if (maximumAutoBidBudget > userBalance) {
      throw new IllegalArgumentException("Ngân sách tối đa không được vượt quá số dư tài khoản ($"
          + NumberUtil.format(userBalance) + ")!");
    }

    if (autoBidIncrementAmount < stepPrice) {
      throw new IllegalArgumentException("Bước giá tự động ít nhất bằng bước giá của sản phẩm ($"
          + NumberUtil.format(stepPrice) + ")!");
    }
  }
}