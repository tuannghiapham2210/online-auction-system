...
public class BidRoomTimerManager {
   ...
   *
   * @param endTimeStr       Chuỗi thời gian kết thúc (định dạng yyyy-MM-dd HH:mm:ss)
   * @param timerLabel       Nhãn hiển thị đồng hồ đếm ngược
   * @param timerLabelTitle  Nhãn hiển thị tiêu đề của đồng hồ
   * @param timeProgressBar  Thanh tiến trình thời gian dạng vùng đồ họa Region
   * @param onEndedCallback  Hành động callback được thực thi khi thời gian kết thúc
   */
  public void startCountdown(
      String endTimeStr,
      Label timerLabel,
      Label timerLabelTitle,
      Region timeProgressBar,
      Runnable onEndedCallback
  ) {
    try {
      stopTimers();
      ...
      if (timeRemaining > 0 && timeProgressBar != null) {
        ...
        progressTimeline = new Timeline(
            new KeyFrame(Duration.ZERO,
                new KeyValue(scaleTransform.xProperty(), percentageRemaining)),
            new KeyFrame(Duration.millis(timeRemaining),
