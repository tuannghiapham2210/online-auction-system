...
public class BidRoomChartManager {
   ...
   *
   * @param priceChart Đối tượng AreaChart cần khởi tạo
   */
   ...
   *
   * @param timeStamp      Mốc thời gian của điểm dữ liệu
   * @param price          Mức giá tại mốc thời gian đó
   * @param isPulseActive Trạng thái kích hoạt hiệu ứng pulse cho node
   */
  public void addDataPoint(String timeStamp, double price, boolean isPulseActive) {
    ...
    if (isPulseActive && customNode != null
        && customNode.getUserData() instanceof ChartNodeController) {
   ...
   *
   * @param price Mức giá hiển thị trên node
   * @return StackPane chứa giao diện của node điểm dữ liệu
   */
   ...
   *
   * @param timeStamp Mốc thời gian gốc cần xử lý trùng lặp
   * @return Chuỗi nhãn thời gian đã được chèn khoảng trống độc nhất
   */
  private String generateUniqueTimeStamp(String timeStamp) {
    ...
    unique.repeat(" ", tickSpaceCounter);
